...
# Chequear validez de la cadena
@app.route('/is_valid', methods=['GET'])
def is_valid():
    is_valid = blockchain.is_chain_valid(blockchain.chain)
    if is_valid:
        response = {'message': 'OK, la cadena de bloques es válida'}
    else:
        response = {'message': 'Atención, la cadena de bloques no es válida'}
    return jsonify(response), 200

# Agregar transacción a la cadena
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message': 'Se remplazó la cadena por la mas larga',
                    'new_chain':blockchain.chain}
    else:
        response = {'message': 'La cadena de bloques es la más larga',
